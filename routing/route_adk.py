...
unclear_agent = Agent(
    name="Unclear",
    model="gemini-2.0-flash",
    description="A specialized agent that handles unclear or ambiguous requests that cannot be properly categorized.",
    tools=[unclear_tool]
)
...
coordinator = Agent(
    ...
    instruction=(
        ...
        "- For general information questions, delegate to the 'Info' agent.\n"
        "- For unclear, ambiguous, or unclassifiable requests, delegate to the 'Unclear' agent."
    ),
...
async def main():
    ...
    result_e = await run_coordinator(runner, "Find month.")  # Should go to Unclear
    print(f"Final Output D: {result_e}")
